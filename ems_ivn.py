	...
	def update ( self , key , data , dec_qty = False , qty=0):
        ...
        def relog ( self ):
		        ...
		        with open('ivn.log','w') as f_obj:
                        ...
                        exit()
        def append_ivn ( self , pid , amt , unit , rate , desc , new=0 ):
                #Checks and adds Content to the inventory
                try:
                        z,k=float(amt),float(rate)
                        del z
                        del k
                except:
                ...
                #Parse them nicely
                try:
                        with open('ivn.log','w' if new else 'a') as f_obj:
                                line = str('%sep'.join([pid,amt,unit,rate,desc])+'\n')
                                f_obj.write(line)
                                f_obj.flush()
                                f_obj.close()
